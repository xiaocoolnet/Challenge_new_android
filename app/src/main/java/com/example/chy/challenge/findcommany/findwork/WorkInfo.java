...
import com.example.chy.challenge.findcommany.findjob.Job_boss_detail;
...
public class WorkInfo extends Activity implements View.OnClickListener{
    ...
    private Find_work_bean.DataBean jobsbean;
...
//                                    send_resume//边框变灰色
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getview();
        intent = getIntent();
        jobsbean = (Find_work_bean.DataBean) intent.getSerializableExtra("jobcompanybean");
        if (jobsbean != null) {
            getinfo();
        }
    }
    ...
    private void getinfo() {
        ...
        if (jobsbean.getCompany_name() != null &&jobsbean.getCompany_name().length() > 0) {
            tv_companyname.setText(jobsbean.getCompany_name());
        }else{
            tv_companyname.setText("");
        }
        if (jobsbean.getLogo() != null&&jobsbean.getLogo().length() > 0) {
            imageLoader.displayImage(NetBaseConstant.NET_HOST + jobsbean.getLogo(), roundImageView, options);
        }else{
...
//        tv_jobtype//职位分类
        if (jobsbean.getJob_count() != null &&jobsbean.getJob_count().length() > 0) {
            tv_job_num.setText("共"+jobsbean.getJob_count()+"个职位");
        }else{
    ...
    public void onClick(View v) {
        switch (v.getId()){
                        ...
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialogone, int which) {
                                ...
                                if (NetBaseUtils.isConnnected(mContext)) {
                                    ...
                                    dialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
                                    dialog.show();
                                    new UserRequest(mContext, handler).APPLYJOB(infoBean.getUserid(),jobsbean.getUserid(),jobsbean.getJobid(),APPLYJOB);
                                }else{
                                    Toast.makeText(mContext, R.string.net_error,Toast.LENGTH_SHORT).show();
                                }
                                dialogone.dismiss();
            ...
            case R.id.job_info_companyinfo:
                ...
                intent.putExtra("companyid",jobsbean.getUserid());
    ...
    private void issendresume() {
        if (NetBaseUtils.isConnnected(mContext)) {
            ...
            dialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
            dialog.show();
            new UserRequest(mContext, handler).APPLYJOB_JUDGE(infoBean.getUserid(),jobsbean.getCompanyid(),jobsbean.getJobid(),APPLYJOB_JUDGE);
