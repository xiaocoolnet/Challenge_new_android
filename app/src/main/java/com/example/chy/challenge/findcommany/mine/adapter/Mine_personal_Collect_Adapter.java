...
public class Mine_personal_Collect_Adapter extends BaseAdapter{
    private List<Mine_personal_Collect_bean.DataBean> blacklist;
    ...
    public Mine_personal_Collect_Adapter(List<Mine_personal_Collect_bean.DataBean> blacklist, Context mContext, int type) {
...
//                    holder.company_name = (TextView) convertView.findViewById(R.id.company_tv_name);//公司名称
        ...
        switch (type){
            case 0:
                holder.work_position.setText(blacklist.get(position).getTitle()+"");
//                holder.company_name.setText(blacklist.get(position).getCompany_name());
                if (blacklist.get(position).getLogo() != null&&blacklist.get(position).getLogo().length() > 0) {
                    imageLoader.displayImage(NetBaseConstant.NET_HOST + blacklist.get(position).getLogo(), holder.adapter_head_image, options);
                }else{
                    holder.adapter_head_image.setImageDrawable(mContext.getResources().getDrawable(R.mipmap.ic_wode));
                }
                holder.company_tv_manager.setText(blacklist.get(position).getRealname()+"");
                holder.manager_position.setText(blacklist.get(position).getMyjob()+"");
                holder.company_tv_salary.setText(blacklist.get(position).getSalary()+"");
                holder.company_tv_count.setText(blacklist.get(position).getCount()+"");
                if (blacklist.get(position).getCity() != null&&blacklist.get(position).getCity().length() > 0){
                    holder.location_city.setText(blacklist.get(position).getCity().substring(0,2)+"");
                }else{
                    holder.location_city.setText("");
                }
                holder.jobtime_worklife.setText(blacklist.get(position).getExperience()+"");
                holder.personal_degree.setText(blacklist.get(position).getEducation()+"");
                holder.work_property.setText(blacklist.get(position).getWork_property()+"");
                break;
        }
        return convertView;
    }
