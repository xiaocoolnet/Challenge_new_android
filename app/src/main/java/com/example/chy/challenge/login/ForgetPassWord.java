...
public class ForgetPassWord extends Activity implements View.OnClickListener{
    ...
    private RevealButton getCode,complete;
    ...
    private void initview() {
        ...
        getCode = (RevealButton) findViewById(R.id.getcode);//获取验证码
...
//        visiblep2.setOnClickListener(this);
        complete = (RevealButton) findViewById(R.id.complete);
...
//                dialog.setProgressStyle(ProgressDialog.STYLE_HORIZONTAL);
            dialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
            dialog.show();
            new UserRequest(mContext, handler).ChangePwd(getPhoneNumber(),inputNewPassword.getText().toString().trim(),code,KEY_CHANGE_PASSWORD);
        }else{
            Toast.makeText(mContext, R.string.net_error,Toast.LENGTH_SHORT).show();
        }
    }
...
//        }
        ...
        if (inputNewPassword2.getText().toString()==null||"".equals(inputNewPassword2.getText().toString().trim())) {
