...
//        rootview = mactivity.getWindow().getDecorView();
    ...
    private void getphotograph() {
        intentFromGallery = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        // 判断存储卡是否可以用，可用进行存储
        String state = Environment.getExternalStorageState();
        if (state.equals(Environment.MEDIA_MOUNTED)) {
            File path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM);
            File file = new File(path, "newpic.jpg");
            intentFromGallery.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(file));
        }
        mactivity.startActivityForResult(intentFromGallery, PHOTO_REQUEST_CAMERA);
    }
